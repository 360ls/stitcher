...
def stitchImages(key_frame_file, base_img_rgb, dir_list, output, round, img_type):
    if ( len(dir_list) < 1 ):
        return base_img_rgb
    ...
    if ( closestImage['inliers'] > 0.1 ): # and 
        ...
        return stitchImages(key_frame_file, final_img, new_dir_list, output, round+1, img_type)

    else:

        return stitchImages(key_frame_file, base_img_rgb, new_dir_list, output, round+1, img_type)
